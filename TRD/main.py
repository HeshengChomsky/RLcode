...
from utils.dataset_utils import processed_qlearning_dataset,my_processed_qlearning_dataset
from utils.filesystem import mkdir


def model_evluation(arg):
    ...
    model_name = None
    deterministic = False
    save_dir = os.path.join(args.save_dir + '_forward_{}'.format(args.seed))
    model = construct_model(obs_dim=obs_dim, act_dim=act_dim, hidden_dim=hidden_dim, num_networks=num_networks,
                            ...
                            name=model_name, deterministic=deterministic,load_dir=save_dir)

    samples = my_processed_qlearning_dataset(env, args.env_name, timeout_frame=False, done_frame=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_name', default='antmaze')
